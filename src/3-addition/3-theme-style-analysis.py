...
class ThemeStyleAnalyzer:
    ...
    def analyze_writing_style(self) -> Dict[str, Dict]:
        ...
        for work_name, segments in self.segmented_texts.items():
            ...
            # 计算平均句长
            style_features[work_name]['avg_sentence_length'] = len(all_words) / len(segments)
            
            # 计算词汇丰富度（Type-Token Ratio）
            unique_words = len(set(all_words))
            total_words = len(all_words)
            style_features[work_name]['vocabulary_richness'] = unique_words / total_words
    ...
    def compare_chapter_styles(self) -> Dict[str, List[float]]:
        ...
        for work_name, segments in self.segmented_texts.items():
            ...
            for i in range(0, len(segments), chunk_size):
                ...
                chapter_features[work_name]['vocabulary_richness'].append(vocab_richness)
    ...
    def visualize_topics(self):
        ...
        for idx, topic_words in enumerate(top_words):
            ...
            # 创建词频分布
            word_freq = {word: (len(top_words) - i) for i, word in enumerate(topic_words)}
    ...
    def visualize_style_features(self, style_features: Dict[str, Dict]):
        ...
        # 1. 平均句长对比
        avg_lengths = [features['avg_sentence_length'] for features in style_features.values()]
        ...
        # 2. 词汇丰富度对比
        richness = [features['vocabulary_richness'] for features in style_features.values()]
        ...
        # 4. 主题分布
        topic_matrix = np.array([self.document_topics[work] for work in style_features.keys()])
        sns.heatmap(topic_matrix, ax=ax4,
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'style_features.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def visualize_chapter_comparison(self, chapter_features: Dict[str, List[float]]):
        ...
        for work_name, features in chapter_features.items():
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f'{work_name}_chapter_comparison.png'), 
...
def segment_text(text: str) -> List[str]:
    ...
    # 过滤停用词和标点符号
    words = [w for w in words if len(w.strip()) > 0 and not w.strip() in '，。！？；：（）【】《》""']
    return words
